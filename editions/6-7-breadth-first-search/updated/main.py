...
def load_graph_from_file(filename):
    ...
    return _add_missing_vertices(graph)


def traversal_distances(graph, source_vertex):
...
def single_edge_distance(graph, source_vertex, destination_vertex):
    found_w = False
    distance = 0
    queue, track = [], []
...
def _add_missing_vertices(graph):
    """
    Takes a graph in the form of a dictionary and verifies
    that for every edge member there exists a source node.
    If no source node is found from existing edge members,
    one is created with an empty edge [].
    :param graph:
    :return: 
    """
...
def main():
    ...
    formatted_shortest = _format_path(shortest)

    distances, traversal_path = traverse(graph, source_vertex)
    formatted_traversal_path = _format_path(traversal_path)
    formatted_distances = _format_distances_table(distances)

    n_hops = single_edge_distance(graph, source_vertex, destination_vertex)
